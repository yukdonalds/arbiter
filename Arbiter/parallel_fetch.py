...
def fetch_daily_metrics_parallel(ib, tickers: list[str], max_tickers: int | None = None) -> dict:
    ...
    return _fetch_daily_metrics_for_end_dt(ib, tickers, end_dt_str, max_tickers, use_backtest_volume_min=False)


def _fetch_daily_metrics_for_end_dt(
    ib, tickers: list[str], end_dt_str: str, max_tickers: int | None = None, use_backtest_volume_min: bool = False, apply_filters: bool = True, progress_callback=None
) -> dict:
    """
    Fetch daily metrics for many tickers in parallel, anchored to a specific end_dt_str.
    Used by backtest for date-specific universe rescan.
    When apply_filters=False (fixed universe), returns metrics for all tickers that have data.
    progress_callback(current, total) called as each ticker completes.
    """
...
def fetch_daily_metrics_parallel_for_date(
    ib, tickers: list[str], end_dt_str: str, max_tickers: int | None = None, use_backtest_volume_min: bool = True, apply_filters: bool = True, progress_callback=None
) -> dict:
    """
    Fetch daily metrics for many tickers in parallel, anchored to a specific date.
    Used by backtest for per-day universe rescan. Returns dict: ticker -> metrics.
    apply_filters=False: return all tickers with data (for fixed universe).
    progress_callback(current, total) optional.
    """
    return _fetch_daily_metrics_for_end_dt(
        ib, tickers, end_dt_str, max_tickers, use_backtest_volume_min=use_backtest_volume_min, apply_filters=apply_filters, progress_callback=progress_callback
    )


def build_watchlist_parallel_for_date(
    ib, tickers: list[str], end_dt_str: str, top_n: int = 100, use_backtest_volume_min: bool = True, progress_callback=None
) -> tuple[list[str], dict]:
    """
    Build watchlist from metrics as of end_dt_str. For backtest per-day rescan.
    progress_callback(current, total) optional.
    """
    max_scan = getattr(config, "FAST_SCAN_MAX_TICKERS", 501)
    metrics = fetch_daily_metrics_parallel_for_date(
        ib, tickers, end_dt_str, max_tickers=max_scan, use_backtest_volume_min=use_backtest_volume_min, progress_callback=progress_callback
    )
