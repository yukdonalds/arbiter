...
def place_partial_runner_exits_side(
    ...
    - TP1: limit for PARTIAL_TP_FRACTION of shares at PARTIAL_TP_PCT
    - Runner: OCA between TP2 (RUNNER_CAP_PCT) and SL (STOP_PCT) for remaining shares
    ...
    cap_pct = float(getattr(config, "RUNNER_CAP_PCT", 0.06))

    if side == "LONG":
        tp1_price = round(fill_price * (1 + tp1_pct), 2)
        tp2_price = round(fill_price * (1 + cap_pct), 2)
        stop_price = round(fill_price * (1 - config.STOP_PCT), 2)
        exit_action = "SELL"
    else:
        tp1_price = round(fill_price * (1 - tp1_pct), 2)
