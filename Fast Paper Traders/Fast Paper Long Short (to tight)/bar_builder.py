# -----------------------------
# Fast Paper Long/Short – Bar builder from 5-sec bars (configurable BAR_MINUTES)
# -----------------------------
...
class BarBuilder:
    def __init__(self):
        self._current = defaultdict(
            lambda: {"open": None, "high": None, "low": None, "close": None, "volume": 0, "start_et": None}
        )
    ...
    def push_ohlcv(self, ticker: str, open_: float, high: float, low: float, close: float, volume: float, t_et: datetime) -> None:
        """Push a full OHLCV bar (e.g. one 5-sec bar). Use for backtest so aggregate bars get true high/low."""
        cur = self._current[ticker]
        if cur["open"] is None:
            cur["open"] = open_
            cur["high"] = high
    ...
    def lock_bar(self, ticker: str, close_time_et: datetime) -> dict | None:
        ...
        self._current[ticker] = {"open": None, "high": None, "low": None, "close": None, "volume": 0, "start_et": None}
        return bar
